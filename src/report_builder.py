...
def build(csv_dir: Path, fig_dir: Path, inputs_dir: Path, report_dir: Path) -> tuple[Path, Path]:
    ...
    add_table(doc, ["1단계: 수요 특성", "2단계: 구간 공백", "3단계: 운영유형"], [["숙박형 활동의 연령·활동·목적·소비 특성 확인", "공식 거리·시간·난이도와 시·종점 1km·3km 접근성 결합", "보급지원형·숙박전환형·기존자원연계형 등 기능별 처방"]], widths=[5.5,5.5,5.5], font=8.0, header_fill="EAF2F8")
    ...
    add_table(doc, ["운영유형", "적용 조건", "우선 조치"], [["보급지원형", "식사 또는 기초 보급 기능이 부족", "예약식사·도시락·식수·충전·영업정보 연계"], ["상업숙박·마을서비스 전환형", "상업숙박은 부족하나 대피소·민박 기반 존재", "민박·대피소·식사·샤워·짐보관 통합"], ["기본관리·기존자원연계형", "이동부담은 높지만 주요 서비스 기반 양호", "신규시설보다 예약·정보·지역업체 연결"], ["교통 추가확인 후보", "정류장 접근은 낮으나 운행정보 미확인", "노선·배차·막차 확인 후 셔틀 실증 여부 판단"]], widths=[4.6,5.8,6.3], font=7.6)
    ...
    add_table(doc, ["분석 단위", "자료", "주요 변수", "산출물"], [["개인", "2025 산림휴양복지활동조사[6]", "당일·숙박 경험, 활동, 목적, 동반, 소비, 인구특성", "숙박형 관련 특성·오즈비"], ["구간", "공식 GPX·코스정보[5]", "거리, 시간, 난이도, 시·종점", "체류·보급 필요도"], ["시설", "숙박·상가·버스 공공데이터[8-10]", "1km·3km 시설 수, 최근접 거리", "기능별 접근성"], ["시·군", "한국관광 데이터랩[7]", "방문, 숙박비율, 체류시간, 소비", "체류시장 여건 보조지표"]], widths=[2.1,4.6,6.0,4.0], font=7.5)
    ...
    add_table(doc, ["지표", "산식·판정", "해석"], [["체류·보급 필요도", "거리 백분위·시간 백분위·난이도 환산값의 평균", "이동부담이 클수록 증가"], ["숙박·보급·교통 접근성", "1km 내 존재=1, 1~3km=0.5, 3km 밖=0", "두 끝점 중 낮은 값 사용"], ["식음 접근성", "1km 3개 이상=1, 1~2개=0.75, 3km 내 3개 이상=0.5, 1~2개=0.25", "단순 존재보다 선택 가능성 반영"], ["기능별 공백", "필요도 × (1-기능별 접근성)", "이동부담 대비 부족한 기능"]], widths=[3.3,7.7,5.6], font=7.5)
    ...
    add_table(doc, ["검증 항목", "처리", "결과"], [["GPX", "공식 시·종점 대조 및 9·55구간 방향 보정", "노선 연결 오류 방지"], ["시설 좌표", "좌표범위·영업상태·중복·행정경계 재검토", "상위구간 시설 접근성 재확인"], ["난이도", "대표값과 최고등급 기준 민감도 분석", "상위 우선순위의 안정성 확인"], ["교통", "정류장 위치와 실제 운행정보 분리", "51구간은 노선·배차 추가확인 후보로 제한"]], widths=[3.0,7.0,6.6], font=7.5, header_fill="EAF2F8")
    ...
    add_table(doc, ["핵심 결과", "당일형", "숙박형", "정책적 의미"], [["1인 1회 평균 소비", f"{day_spend:.1f}만원", f"{overnight_spend:.1f}만원", "숙박형이 약 2.9배"], ["지역 선택", "교통 편의 비중 높음", "자연경관·먹거리·관광연계 비중 높음", "식사·체험 연계 필요"], ["주요 숙박", "-", "호텔·콘도 31.7%, 펜션 31.7%", "상업숙박 수요가 중심"], ["산촌마을·민박", "-", "4.7%", "신규 공급보다 예약·서비스 연계 우선"]], widths=[3.6,3.5,5.0,4.3], font=7.4)
    doc.add_page_break()
    ...
    add_table(doc, ["기존 접근", "본 분석의 변화"], [["시설 수가 적은 지역을 막연히 취약하다고 판단", "이동부담에 비해 어느 기능이 비어 있는지를 구간 양 끝점에서 비교"], ["숙박·식당·교통을 한꺼번에 신규 설치", "보급지원·숙박전환·기존자원연계 등 구간별 최소 기능을 우선 적용"], ["지역 관광규모를 트레일 수요로 직접 해석", "개인·구간·시·군 자료를 분리하고 지역지표는 보조판단에만 사용"], ["정적 시설배치 계획", "개통 전 진단→시범운영→성과자료 반영→재진단의 환류체계"]], widths=[7.7,8.8], font=7.8)
